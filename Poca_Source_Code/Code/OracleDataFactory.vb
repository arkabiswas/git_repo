...
#Region "Helper Functions"
        ...
        Public Shared Function GetDataSourceName(ByVal dataSourceIds As String) As String
            ...
            Dim deletedRecord As String = "Deleted Record"
            Dim dataSourceDescription As String = ""

            ' Loop through result ids with For Each
            Dim ids As String() = dataSourceIds.Split(New Char() {";"c})
            Dim id As String
            Dim desc As String = ""

            For Each id In ids
                If HttpContext.Current.Cache("datasourceName" + id) Is Nothing Then

                    dataSourceList = SearchEngineData.BindList("record_source", po.UserName)
                    While dataSourceList.Read()
                        If dataSourceList("pick_list_item_id").ToString() = id Then
                            desc = dataSourceList("description")
                        End If
                    End While
                    dataSourceList.Close()

                    ' let's add this description to the cache.
                    HttpContext.Current.Cache("datasourceName" + id) = desc
                Else
                    desc = HttpContext.Current.Cache("datasourceName" + id)
                End If

                If dataSourceDescription = "" Then
                    dataSourceDescription = desc
                Else
                    dataSourceDescription = dataSourceDescription + "; " + desc
                End If
            Next

            If dataSourceDescription = "" Then
                dataSourceDescription = deletedRecord
            End If
        ...
        'Jayesh - change name to _Org
        Public Shared Function GetDataSourceName_Org(ByVal DataSourceId As String) As String
            Dim po As FDA.Person.PersonObject = HttpContext.Current.Session("LoggedInUser")
            Dim dataSourceList As Oracle.DataAccess.Client.OracleDataReader
